        ...
        <div class="mx-auto w-fit text-xs">
            <table class="mb-2">
                <tbody class="biodata">
                    <tr>
                        ...
                        <td>Kelas</td>
                        <td>:</td>
                        <td>{{ $siswa->kelas->tingkat_kelas }}</td>
                    </tr>
                    <tr>
            ...
            <ol class="alpha max-w-[950px] space-y-2">
                <li class="">
                    ...
                    <div class="w-full border border-black p-1 pb-6">
                        ...
                        <p>
                            {{ $siswa->rapor->where('tahun_ajaran_id', $siswa->kelas->tahunAjaran->id)->first()->sikap_spiritual ?? '-' }}
                        </p>
                    </div>
                </li>
                <li class="">
                    ...
                    <table class="rapor">
                        ...
                        <tbody>
                            @php
                                $numberI = 1;
                            @endphp
                            @foreach ($siswa->rapor->where('tahun_ajaran_id', $siswa->kelas->tahunAjaran->id)->first()->mapelRapor as $i => $mapelRapor)
                        ...
                        <tbody>
                            @php
                                $numberJ = 1;
                            @endphp
                            @foreach ($siswa->rapor->where('tahun_ajaran_id', $siswa->kelas->tahunAjaran->id)->first()->mapelRapor as $j => $mapelRapor)
                ...
                <li class="!mt-4" style="margin-top: 16px;">
                    ...
                    <table class="ekskul w-full text-center">
                        ...
                        <tbody>
                            @forelse ($siswa->rapor->where('tahun_ajaran_id', $siswa->kelas->tahunAjaran->id)->first()->prestasi as $i => $prestasi)
                                <tr>
                                    <td class="w-7">{{ ++$i }}</td>
                ...
                <li class="!mt-4">
                    ...
                    <table class="ekskul w-full text-center">
                        ...
                        <tbody>
                            @php
                                $totalNilaiC3 = $siswa->rapor->where('tahun_ajaran_id', $siswa->kelas->tahunAjaran->id)->first()->mapelRapor()->join('nilai_pengetahuan_c3_s', 'mapel_rapors.nilai_pengetahuan_c3_s_id', 'nilai_pengetahuan_c3_s.id')->sum('nilai_pengetahuan_c3_s.nilai');
                                $totalNilaiC4 = $siswa->rapor->where('tahun_ajaran_id', $siswa->kelas->tahunAjaran->id)->first()->mapelRapor()->join('nilai_keterampilan_c4_s', 'mapel_rapors.nilai_keterampilan_c4_s_id', 'nilai_keterampilan_c4_s.id')->sum('nilai_keterampilan_c4_s.nilai');

                                $totalNilaiAvg = collect([
                                    $siswa->rapor->where('tahun_ajaran_id', $siswa->kelas->tahunAjaran->id)->first()->mapelRapor()->join('nilai_pengetahuan_c3_s', 'mapel_rapors.nilai_pengetahuan_c3_s_id', 'nilai_pengetahuan_c3_s.id')->avg('nilai_pengetahuan_c3_s.nilai'),
                                    $siswa->rapor->where('tahun_ajaran_id', $siswa->kelas->tahunAjaran->id)->first()->mapelRapor()->join('nilai_keterampilan_c4_s', 'mapel_rapors.nilai_keterampilan_c4_s_id', 'nilai_keterampilan_c4_s.id')->avg('nilai_keterampilan_c4_s.nilai'),
                                ])->filter()->avg();
                            @endphp
                            <tr>
                                <td class="w-7">1</td>
                                ...
                                <td rowspan="2">{{ $siswa->rapor->where('tahun_ajaran_id', $siswa->kelas->tahunAjaran->id)->first()->peringkat }}</td>
                                <td rowspan="2">{{ $siswa->kelas->siswa->count() }}</td>
                            </tr>
                            <tr>
                                <td class="w-7">2</td>
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td class="w-[12%] border-y border-black text-center">
                                    {{ $siswa->rapor->where('tahun_ajaran_id', $siswa->kelas->tahunAjaran->id)->first()->sakit ?? 0 }}
                                </td>
                            ...
                            <tr>
                                ...
                                <td class="w-[2%] border-y border-black">:</td>
                                <td class="w-[12%] border-y border-black text-center">
                                    {{ $siswa->rapor->where('tahun_ajaran_id', $siswa->kelas->tahunAjaran->id)->first()->izin ?? 0 }}
                                </td>
                                <td class="w-[32%] border-y border-r border-black text-start">Hari</td>
                            </tr>
                            <tr>
                                ...
                                <td class="w-[2%] border-y border-black">:</td>
                                <td class="w-[12%] border-y border-black text-center">
                                    {{ $siswa->rapor->where('tahun_ajaran_id', $siswa->kelas->tahunAjaran->id)->first()->tanpa_keterangan ?? 0 }}
                                </td>
                                <td class="w-[32%] border-y border-r border-black text-start">Hari</td>
                            </tr>
                        </tbody>
                ...
                <li class="!mt-4">
                    ...
                    <p class="w-full border border-black p-3 text-center italic font-bold">
                        {{ $siswa->rapor->where('tahun_ajaran_id', $siswa->kelas->tahunAjaran->id)->first()->catatan_wali_kelas ?? "Semangat belajarnya lebih ditingkatkan lagi agar dapat memperoleh prestasi yang lebih baik" }}
                    </p>
                </li>
            </ol>
            <section class="mt-8 flex justify-around text-center">
                ...
                <div class="flex flex-col gap-20">
                    <h4>Wali Kelas</h4>
                    <span>
                        <p class="underline">{{ $siswa->kelas->guru->user->name }}</p>
                        <p>NPY.{{ $siswa->kelas->guru->npy }}</p>
                    </span>
                </div>
                <div class="flex flex-col gap-20">
                    <span>
                        <h4>{{ $siswa->kelas->tahunAjaran->tempat_pembagian }}, {{ date('d F Y', strtotime($siswa->kelas->tahunAjaran->tanggal_pembagian)) }}</h4>
                        <p>Orang Tua/Wali</p>
                    </span>
